...
public final class Partido implements PartidoInterface {
    ...
    private Position posLibreIndirecto = new Position(); //Posición desde la que se debe sacar la falta.
    private ArrayList<Integer>distanciaSaqueInsuficiente; //Array que contiene los jugadores que se hallan demasiado próximos al balón.
  ...
  /** Itera en un saque hasta que los jugadores se han retirado lo suficiente del balón para respetar con la distancia mínima para el saque**/
   private void iteraDistanciaMinima() {
	...
	if (distanciaSaqueInsuficiente.size() == 0)
	   ...
	   corregirDistanciaSaque(); //Mueve a los jugadores fuera de la distancia mínima de saque
  }
        ...
        if (!sacaLocal && !sacaVisita) {
            ...
            if (iteracionSaque > Constants.ITERACIONES_SAQUE) {
                if (anteriorCambio == iteracion) {
                    ...
                    cambioSaque = true;
                    estado = 8; //Comprobamos la distancia de separación
       ...
       //recorre los comandos del local
        ...
        if (listGolpearBalon.size() > 0) {//si existe al menos un comando golpear ballPosition
            ...
            if (comandoGolpearOk) {
                ...
                //Si el jugador que va ha disparar es del equipo que golpeo el balon la última vez y no ha sido el mismo jugador el que ha golpeado anteriormente (pues entonces no seria fuera juego sino un autopase).
               ...
               calculateOffSidePlayers(ce.eq == 0); //Rellenamos el array con la información de fuera de juego
    ...
    /**
     * Calcula los jugadores que estan en el pase actual y rellena el array outOfGamePlayers con dicha información.
    ...
    void calculateOffSidePlayers(boolean isLocal) {
        ...
        if (posOffSide == null) posOffSide = balon;//Si no hay ningún defensor por delante el fuera de juego lo marca el balon
        ...
        for (int x = 0; x < 11; ++x) {
            ...
            if (isLocal)
                 offSidePlayers[x] = (positionPlayers[x].getY() > posOffSide.getY() && !dentroPorteria && (estadoSaque != SAQUE_EN_EJECUCION) && ( (ultimoSaque != SAQUE_FONDO) || (ultimoSaque != SAQUE_BANDA) ));
            else
                offSidePlayers[x] = (positionPlayers[x].getY() < posOffSide.getY() && !dentroPorteria && (estadoSaque != SAQUE_EN_EJECUCION) && ( (ultimoSaque != SAQUE_FONDO) || (ultimoSaque != SAQUE_BANDA) ));
    ...
    {              
        ...
        Position posBalonNoSaca = (sacaLocal) ? balon.getInvertedPosition() : balon;
         ...
         //Movemos a los jugadores del equipo que no saca que se hallen dentro de la distancia mínima para el saque
        ...
        { 
            ...
            //Si la posicion resultante está fuera del campo cambiamos la direccion
            if (!p.isInsideGameField(0))
            {
            	if ( (Constants.ANCHO_CAMPO_JUEGO / 2) - Math.abs( p.getX() ) < Constants.DISTANCIA_SAQUE + 1)
            	{
            		if (p.getX() > 0)
            			p = p.setPosition( p.getX() - (Constants.DISTANCIA_SAQUE + 1), p.getY());
            		else
            			p = p.setPosition( p.getX() + (Constants.DISTANCIA_SAQUE + 1), p.getY());
            	}
            	
            	if (  (Constants.LARGO_CAMPO_JUEGO / 2) - Math.abs( p.getY() ) < Constants.DISTANCIA_SAQUE + 1)
            	{
            		if (p.getX() > 0)
            			p = p.setPosition( p.getY() - (Constants.DISTANCIA_SAQUE + 1), p.getY());
            		else
            			p = p.setPosition( p.getY() + (Constants.DISTANCIA_SAQUE + 1), p.getY());
            	}
            }
        ...
        {
        	//Si no es del tipo MoveTo lo saltamos
        	if (cmd.getCommandType() == CommandType.HIT_BALL) continue;
        	
        	//Si es del tipo irA lo ejecutamos
		
          	//modificamos la posicion del jugador
        	
        	posJugadoresSaca[cmd.getPlayerIndex()] = irA( tacticaSaca, cmd.getPlayerIndex(), posJugadoresSaca[cmd.getPlayerIndex()], ( (CommandMoveTo) cmd).getMoveTo(), false);
    ...
    /**Reducimos la energía del jugador**/
    ...
    /**Incrementamos la energía del jugador**/
    ...
    private void actualizarEnergia(List <Command> comandosLocales, List <Command> comandosVisita){
      ...
      //Incrementamos la energia a aquellos jugadores que no se les ha asignado ningún comando
