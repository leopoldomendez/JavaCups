...
public class MovimientoCoberturaLimitada {
	...
	public List<Command> fijarCoberturas(GameSituations gs) {
		...
		if(!coberturasFijadas && gs.rivalPlayers()[indiceDelanteroCentroRival].getY() < (-Constants.LARGO_CAMPO_JUEGO / 6)) {
			for(int i = 0; i < 4; i++)
		...
		if(coberturasFijadas) {
			...
			for(int i = 0; i < 11; i++) {
				...
				sumatorioDistancias.put(i, sumatorioJugador + gs.rivalPlayers()[i].distance(Constants.centroArcoInf));
			}
